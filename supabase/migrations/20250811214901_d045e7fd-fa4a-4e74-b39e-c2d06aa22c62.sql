-- Fix policies creation by separating commands
-- Create billing table if missing
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='billing' AND policyname='Users can select own billing'
  ) THEN
    CREATE POLICY "Users can select own billing" ON public.billing
      FOR SELECT USING (user_id = auth.uid());
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='billing' AND policyname='Admins can select all billing'
  ) THEN
    CREATE POLICY "Admins can select all billing" ON public.billing
      FOR SELECT USING (public.has_role(auth.uid(), 'admin'));
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='billing' AND policyname='Users can insert own billing'
  ) THEN
    CREATE POLICY "Users can insert own billing" ON public.billing
      FOR INSERT WITH CHECK (user_id = auth.uid());
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='billing' AND policyname='Users can update own billing'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='presence' AND policyname='Users can upsert own presence insert'
  ) THEN
    CREATE POLICY "Users can upsert own presence insert" ON public.presence
      FOR INSERT WITH CHECK (user_id = auth.uid());
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='presence' AND policyname='Users can upsert own presence update'
  ) THEN
    CREATE POLICY "Users can upsert own presence update" ON public.presence
      FOR UPDATE USING (user_id = auth.uid());
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='presence' AND policyname='Admins can select all presence'
  ) THEN
    CREATE POLICY "Admins can select all presence" ON public.presence
      FOR SELECT USING (public.has_role(auth.uid(), 'admin'));
  END IF;
END $$;
...
DECLARE
  ...
  active_now bigint;
BEGIN
  start_today_utc := date_trunc('day', now() AT TIME ZONE tz) AT TIME ZONE tz;
  ...
  FROM public.billing b
  WHERE b.plan_price_id = price_id AND b.status = 'active'
    AND (b.trial_end IS NULL OR b.trial_end <= now());

  SELECT COUNT(*) INTO premium_converted_today
  FROM public.billing b
  WHERE b.plan_price_id = price_id AND b.premium_activated_at IS NOT NULL
    AND b.premium_activated_at >= start_today_utc AND b.premium_activated_at < now();

  SELECT COUNT(*) INTO premium_converted_last7
  FROM public.billing b
  WHERE b.plan_price_id = price_id AND b.premium_activated_at IS NOT NULL
    AND b.premium_activated_at >= start_last7_utc AND b.premium_activated_at < now();
