import React, { useState } from 'react';
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card';
import { Button } from '@/components/ui/button';
import { Badge } from '@/components/ui/badge';
import { Crown, Heart, Sparkles, Check, X, Smartphone, Globe } from 'lucide-react';
import { HeartIcon } from '@/components/ui/heart-icon';
import { usePlatformPayments } from '@/hooks/usePlatformPayments';
import { useToast } from '@/hooks/use-toast';

interface PaywallProps {
  onPlanSelected: () => void;
  onSkipToFree?: () => void;
  isModal?: boolean;
}

const Paywall: React.FC<PaywallProps> = ({ onPlanSelected, onSkipToFree, isModal = false }) => {
  const [isLoading, setIsLoading] = useState(false);
  const [selectedPlan, setSelectedPlan] = useState<'single' | 'split'>('single');
  const { 
    isNativeMobile, 
    processPayment, 
    currentPlatform,
    isProcessing 
  } = usePlatformPayments();
  const { toast } = useToast();

  const handlePayment = async (paymentPlan: 'single' | 'split') => {
    setIsLoading(true);
    try {
      if (isNativeMobile) {
        // For mobile apps, redirect to app store
        const platform = currentPlatform;
        let storeUrl = '';
        
        if (platform === 'ios') {
          storeUrl = 'https://apps.apple.com/app/purposely/id123456789'; // Replace with actual App Store ID
        } else if (platform === 'android') {
          storeUrl = 'https://play.google.com/store/apps/details?id=com.purposely.app'; // Replace with actual package name
        }
        
        if (storeUrl) {
          window.open(storeUrl, '_blank');
          toast({
            title: "Redirecting to Store",
            description: `Opening ${platform === 'ios' ? 'App Store' : 'Google Play'} to complete your purchase...`,
          });
        }
      } else {
        // For web, use platform payment processing (which will use Stripe)
        const result = await processPayment(paymentPlan);
        if (result.success) {
          onPlanSelected();
        } else {
          throw new Error(result.error || 'Payment failed');
        }
      }
    } catch (error) {
      console.error('Error processing payment:', error);
      toast({
        title: "Error",
        description: "Failed to process payment. Please try again.",
        variant: "destructive",
      });
    } finally {
      setIsLoading(false);
    }
  };

  const features = [
    "Unlimited conversation starters",
    "AI-powered text message help",
    "Red Flag Detector", 
    "Therapy companion tools",
    "Mental health check-ins",
    "Growth journal",
    "Premium AI insights",
    "Priority support"
  ];

  const content = (
    <div className={`${isModal ? 'max-w-2xl' : 'max-w-2xl'} w-full space-y-6`}>
      {/* Header */}
      <div className="text-center space-y-4">
        <div className="flex justify-center">
          <div className="w-16 h-16 bg-gradient-romance rounded-full flex items-center justify-center shadow-glow">
            <HeartIcon className="w-8 h-8 text-white" size={32} />
          </div>
        </div>
        <h1 className="text-3xl font-bold bg-gradient-romance bg-clip-text text-transparent">
          {isModal ? 'Unlock Premium Features' : 'Welcome to Purposely Premium'}
        </h1>
        <p className="text-muted-foreground leading-relaxed">
          You know what you want, now let our expert-guided journey ensure that you get it. Dating should come with depth, not deja vu.
        </p>
      </div>

      {/* Payment Options */}
      <div className="grid gap-4 md:grid-cols-2">
        {/* Single Payment Option */}
        <Card className={`shadow-romance border-primary/20 relative overflow-hidden cursor-pointer transition-all ${selectedPlan === 'single' ? 'ring-2 ring-primary' : ''}`}
              onClick={() => setSelectedPlan('single')}>
          <div className="absolute top-0 left-0 right-0 h-1 bg-gradient-romance"></div>
          
          <CardHeader className="text-center space-y-4">
            <div className="flex justify-center">
              <Badge variant="secondary" className="bg-gradient-romance text-white border-0 px-4 py-1">
                Best Value
              </Badge>
            </div>
            
            <CardTitle className="text-xl font-bold">Single Payment</CardTitle>
            
            <div className="space-y-2">
              <div className="flex items-center justify-center space-x-2">
                <span className="text-3xl font-bold text-primary">$1,497</span>
              </div>
              <p className="text-sm text-muted-foreground">
                One-time payment • Lifetime access
              </p>
            </div>
          </CardHeader>

          <CardContent className="space-y-4">
            {/* Features List */}
            <div className="space-y-2">
              {features.slice(0, 4).map((feature, index) => (
                <div key={index} className="flex items-center space-x-2">
                  <Check className="w-4 h-4 text-green-500 flex-shrink-0" />
                  <span className="text-xs">{feature}</span>
                </div>
              ))}
              <div className="text-xs text-muted-foreground">+ 4 more features</div>
            </div>
          </CardContent>
        </Card>

        {/* Split Payment Option */}
        <Card className={`shadow-romance border-primary/20 relative overflow-hidden cursor-pointer transition-all ${selectedPlan === 'split' ? 'ring-2 ring-primary' : ''}`}
              onClick={() => setSelectedPlan('split')}>
          <div className="absolute top-0 left-0 right-0 h-1 bg-gradient-romance"></div>
          
          <CardHeader className="text-center space-y-4">
            <div className="flex justify-center">
              <Badge variant="outline" className="border-primary text-primary px-4 py-1">
                Payment Plan
              </Badge>
            </div>
            
            <CardTitle className="text-xl font-bold">Split Payment</CardTitle>
            
            <div className="space-y-2">
              <div className="flex items-center justify-center space-x-2">
                <span className="text-3xl font-bold text-primary">$997</span>
              </div>
              <p className="text-sm text-muted-foreground">
                2 payments • Second payment in 30 days
              </p>
            </div>
          </CardHeader>

          <CardContent className="space-y-4">
            {/* Features List */}
            <div className="space-y-2">
              {features.slice(0, 4).map((feature, index) => (
                <div key={index} className="flex items-center space-x-2">
                  <Check className="w-4 h-4 text-green-500 flex-shrink-0" />
                  <span className="text-xs">{feature}</span>
                </div>
              ))}
              <div className="text-xs text-muted-foreground">+ 4 more features</div>
            </div>
          </CardContent>
        </Card>
      </div>

      {/* Platform indicator */}
      {isNativeMobile && (
        <div className="flex items-center justify-center space-x-2 text-sm text-muted-foreground">
          <Smartphone className="w-4 h-4" />
          <span>
            {currentPlatform === 'ios' ? 'App Store' : 'Google Play'} Purchase
          </span>
        </div>
      )}

      {/* CTA Button */}
      <Button
        onClick={() => handlePayment(selectedPlan)}
        disabled={isLoading || isProcessing}
        className="w-full bg-gradient-romance hover:opacity-90 text-white font-semibold py-3 text-lg shadow-lg hover:shadow-xl transition-all duration-300"
      >
        {(isLoading || isProcessing) ? (
          <div className="flex items-center space-x-2">
            <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-white"></div>
            <span>
              {isNativeMobile ? 'Opening Store...' : 'Processing...'}
            </span>
          </div>
        ) : (
          <>
            <HeartIcon className="w-5 h-5 mr-2" size={20} />
            {isNativeMobile 
              ? `Get Premium via ${currentPlatform === 'ios' ? 'App Store' : 'Google Play'}`
              : `Pay ${selectedPlan === 'single' ? '$1,497' : '$997'} Now`
            }
          </>
        )}
      </Button>

      {/* No Thank You Option */}
      {onSkipToFree && (
        <div className="text-center pt-4">
          <button
            onClick={onSkipToFree}
            className="text-sm text-muted-foreground hover:text-foreground transition-colors underline"
          >
            Take A Sneak Peak Inside
          </button>
        </div>
      )}

      {/* Trust Indicators */}
      <div className="text-center space-y-2 pt-4 border-t border-border">
        <p className="text-xs text-muted-foreground">
          ✓ Secure payment • ✓ Lifetime access • ✓ Money-back guarantee
        </p>
        <p className="text-xs text-muted-foreground">
          Unlock all premium features with a single purchase
        </p>
      </div>

      {/* Bottom Text */}
      {!isModal && (
        <div className="text-center">
          <p className="text-xs text-muted-foreground">
            By continuing, you agree to our Terms of Service and Privacy Policy
          </p>
        </div>
      )}
    </div>
  );

  if (isModal) {
    return content;
  }

  return (
    <div className="min-h-screen bg-gradient-soft flex items-center justify-center p-4">
      {content}
    </div>
  );
};

export default Paywall;