...
Format as: Statement? followed by A. [option] B. [option] C. [option] D. [option]`;
  ...
  const loadMoreStarters = async () => {
    if (isCustom && customCategories[selectedCategory]) {
      ...
      const prompt = `Generate 25 new conversation starter questions similar to these but completely different: ${existingQuestions.join(', ')}`;
      try {
      ...
      if (category) {
        ...
        if (selectedCategory === 'Trust & Transparency') {
          ...
          prompt = `Create 25 intimate, adult conversation starters that blend vulnerability with sensual undertones. Match the tone and structure of these examples without repeating them: "What’s one way I’ve touched you in the past that you still think about?", "When do you feel most wanted by me?", "If we could relive one intimate moment together, which would you choose?", "What’s one small thing I could do tonight that would make you feel completely desired?" Use emotionally charged yet tasteful language, hinting at desire without explicit detail. Vary openings (what/when/which/if you could/describe/where/how), use second-person or "we", add subtle sensory cues (warmth, breath, closeness, lingering), and keep consent-forward phrasing. Each item must be exactly one sentence, a single question ending with a question mark, unnumbered, and avoid repeating the same first three words across items.`;
